...
namespace Adf.Core.IoC
{
    ...
    public static class AutoRegisterServicesExtensions
    {
        ...
        public static IServiceCollection AsConcreteTypesTransient(this AutoRegisterData autoRegData)
        {
            if (autoRegData == null) throw new ArgumentNullException(nameof(autoRegData));
            foreach (var classType in autoRegData.TypeFilter == null
                ? autoRegData.TypesToConsider
                : autoRegData.TypesToConsider.Where(autoRegData.TypeFilter))
            {
                autoRegData.Services.AddTransient(classType);
            }
        ...
        /// <summary>
        /// register concrete types as Scoped
        /// </summary>
        public static IServiceCollection AsConcreteTypesScoped(this AutoRegisterData autoRegData)
        {
            if (autoRegData == null) throw new ArgumentNullException(nameof(autoRegData));
            foreach (var classType in autoRegData.TypeFilter == null
                ? autoRegData.TypesToConsider
                : autoRegData.TypesToConsider.Where(autoRegData.TypeFilter))
            {
                autoRegData.Services.AddScoped(classType);
            }

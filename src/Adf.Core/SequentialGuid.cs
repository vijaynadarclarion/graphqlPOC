...
namespace Adf.Core.Utils
{
    ...
    public sealed class SequentialGuid
    {
        ...
        public static Guid NewGuid()
        {
            ...
            if (!BitConverter.IsLittleEndian) Array.Reverse(ticks);

            return new Guid(
                new[]
                    {
                        guid[0], guid[1], guid[2], guid[3], guid[4], guid[5], guid[6], guid[7], ticks[1], ticks[0],
                        ticks[7], ticks[6], ticks[5], ticks[4], ticks[3], ticks[2]
                    });
        }
        ...
        /// <summary>
        /// Converts the string representation of a GUID to the equivalent <see cref="T:System.Guid"/> structure, provided
        ...
        /// </param>
        /// <param name="result">
        /// The structure that will contain the parsed value. If the method returns true, <paramref name="result"/> contains a
        ///     valid <see cref="T:System.Guid"/>. If the
        ...
        {
            if (_counter == 0) _counter = DateTime.UtcNow.Ticks;
